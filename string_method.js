...
//10. Below is the bookChapters array that contains inner arrays. Each inner array consists of a chapter's title, and the number of a page,
//  for example, in ['The Basics', 14], 'The Basics' is the chapter's title, and 14 is the number of a page.
...
function logBookChapters(array){
...
//     const text = rows[i].trim().toLowerCase()
//     // console.log(rows[i])//'underscore_case' 
//     const spaceInd = text.indexOf('_')
//     const camelCase = text.slice(0,spaceInd) + text[spaceInd+1].toUpperCase() + text.slice(spaceInd+2)
//     //console.log(camelCase)//'underscoreCase'
//     // checked
//     const checked = '✅'.repeat(i+1)
//     console.log(camelCase.padEnd(25,' ')+checked)
//   }
// }
function toCamelCase(text){
  const rows = text.split('\n')
  ...
  for (let i = 0; i < rows.length; i++){
    ...
    //console.log(first, second)// 'underscore', 'case' 
    const camelCaseLetter = first + second.replace(second[0], second[0].toUpperCase())
